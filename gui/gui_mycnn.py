...
class DigitRecognizerApp:
    def __init__(self, root):
        self.root = root
        self.root.title("Rozpoznawanie Cyfr (Mój Własny CNN)")
    ...
    def predict_digit(self):
        if self.model is None:
            self.label_result.config(text="Błąd: Brak modelu! Wykonaj wpierw train.py")
            return
        ...
        # Rescaling with MNIST standard (width=20px)
